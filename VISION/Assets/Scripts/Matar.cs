...
public class Matar   : MonoBehaviour {
	...
	{
		if (damageOnTrigger) {
			if (this.tag == "PlayerBullet" && collision.gameObject.tag == "Player" && collision.gameObject.tag == "Finish")	// if the player got hit with it's own bullets, ignore it
				return;
	...
	{
		if (continuousDamage) {
			if (collision.gameObject.tag == "Finish" && collision.gameObject.tag == "Player" && collision.gameObject.GetComponent<Salud> () != null) {	// is only triggered if whatever it hits is the player
